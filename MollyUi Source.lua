--[[ MollyUi Library
    A Roblox Luau UI library with anti-cheat evasion techniques
    Based on Splix Lib architecture with security enhancements
]]

-- // Services with cloneref support
...
-- // UI Variables with anti-cheat evasion
local library = {
    drawings = {},
    hidden = {},
    connections = {},
    ...
    folders = {
        main = "mollyui",
        assets = "mollyui/assets",
        configs = "mollyui/configs"
    },
    shared = {
        initialized = false,
        fps = 0,
...
-- // Theme Variables with clean naming
local theme = {
...
-- // Anti-cheat: Utility Functions with security enhancements
do
    ...
    function utility:Create(instanceType, instanceOffset, instanceProperties, instanceParent)
        ...
        -- Anti-cheat: Stagger drawing creation to avoid detection
        local shouldStagger = library.shared.initialized
        if shouldStagger then
            task.wait(math.random() * 0.01) -- Micro-delay between drawing creation
        end
    ...
    function utility:Connection(connectionType, connectionCallback)
        -- Anti-cheat: Wrap callback in newcclosure if available
        local wrappedCallback = newcclosure and newcclosure(connectionCallback) or connectionCallback
    ...
    function utility:LoadImage(instance, imageName, imageLink)
        ...
        if success and data and instance then
            instance.Data = data
        end
    end
    ...
    -- Anti-cheat: Rate limiting utility
    function utility:CheckRateLimit(remoteName, cooldown)
        local currentTime = tick()
        local lastCall = library.remoteCooldowns[remoteName] or 0
        
        if currentTime - lastCall >= cooldown then
            library.remoteCooldowns[remoteName] = currentTime
            return true
        end
        return false
    end
    
    -- Anti-cheat: Sequence number tracking
    function utility:GetNextSequence(remoteName)
        if not library.sequenceNumbers[remoteName] then
            library.sequenceNumbers[remoteName] = 0
        end
...
-- // Library Functions with anti-cheat evasion
do
    ...
    function library:New(info)
        local info = info or {}
        local name = info.name or info.Name or info.title or info.Title or "MollyUi"
        ...
        -- Anti-cheat: Staged window creation
        library.shared.loadingStage = 1
        ...
        function window:Unload()
            -- Anti-cheat: Proper cleanup with error handling
            for i, v in pairs(library.connections) do
                local success, err = pcall(function()
                    v:Disconnect()
                end)
        ...
        function window:Watermark(info)
            ...
            local watermark_name = info.name or info.Name or info.title or info.Title or 
                string.format("MollyUi || uid : %u || ping : %u || fps : %u", 1, 100, 200)
            ...
            local watermark_title = utility:Create("TextLabel", {Vector2.new(2 + 6, 4), watermark_outline}, {
                Text = string.format("MollyUi - fps : %u - uid : %u", 35, 2),
                Size = theme.textsize,
                Font = theme.font,
                Color = theme.textcolor,
                OutlineColor = theme.textborder,
            ...
            utility:Connection(rs.RenderStepped, function(fps)
                local success, fps_val = pcall(function()
                    library.shared.fps = math.round(1 / fps)
                end)
                
                local success, ping_val = pcall(function()
                    library.shared.ping = tonumber(string.split(stats.Network.ServerStatsItem["Data Ping"]:GetValueString(), " ")[1] .. "")
                end)
            end)
            
            watermark_title.Text = string.format("MollyUi || uid : %u || ping : %i || fps : %u", 1, tostring(library.shared.ping), library.shared.fps)
        ...
        function window:Initialize()
            -- Anti-cheat: Staged initialization with random delays
            library.shared.loadingStage = 3
            ...
            if window.dragging and window.isVisible then
                ...
                if utility:GetScreenSize().Y - main_frame.Size.Y - 5 > 5 then
                    local move = Vector2.new(
                        math.clamp(mouseLocation.X - window.drag.X, 5, utility:GetScreenSize().X - main_frame.Size.X - 5),
                        math.clamp(mouseLocation.Y - window.drag.Y, 5, utility:GetScreenSize().Y - main_frame.Size.Y - 5)
                    )
                    window:Move(move)
                else
    ...
    function pages:Section(info)
        ...
        local section_inline = utility:Create("Frame", {
            ...
            Position = utility:Position(
                side == "right" and 0.5 or 0,
                side == "right" and 2 or 5,
                0,
                5 + page.sectionOffset[side],
                window.tab_frame
            ),
            Color = theme.inline,
            Visible = page.open
    ...
    function sections:Toggle(info)
        ...
        function toggle:Set(bool)
            if bool or not bool then
                ...
                -- Anti-cheat: Wrap callback in pcall
                local success, err = pcall(function()
        ...
        library.began[#library.began + 1] = function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and toggle_outline.Visible and window.isVisible and page.open and 
               utility:MouseOverDrawing({
        ...
        function toggle:Colorpicker(info)
            ...
            local colorpicker_outline = utility:Create("Frame", {
                Vector2.new(section.section_frame.Size.X - (toggle.colorpickers == 0 and (30 + 4) or (64 + 4)), colorpicker.axis),
                section.section_frame
            }, {
            ...
            library.began[#library.began + 1] = function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 and window.isVisible and colorpicker_outline.Visible then
                    ...
                    elseif utility:MouseOverDrawing({
                        section.section_frame.Position.X + (section.section_frame.Size.X - (colorpicker.index == 0 and (30 + 4 + 2) or (64 + 4 + 2))),
    ...
    function sections:Slider(info)
        ...
        library.began[#library.began + 1] = function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and slider_outline.Visible and window.isVisible and page.open and 
               utility:MouseOverDrawing({
                   section.section_frame.Position.X,
                ...
                elseif utility:MouseOverDrawing({
                    ...
                    if not multibox.open then
                        ...
                        for i, v in pairs(options) do
                            ...
                            multibox.holder.buttons[#multibox.holder.buttons + 1] = {multibox_value, multibox_value_frame}
                        end
    ...
    function sections:Colorpicker(info)
        ...
        local colorpicker_outline = utility:Create("Frame", {
            Vector2.new(section.section_frame.Size.X - (30 + 4), colorpicker.axis),
            section.section_frame
        }, {
            Size = utility:Size(0, 30, 0, 15),
        ...
        local colorpicker_title = utility:Create("TextLabel", {
            Vector2.new(4, colorpicker.axis + (15 / 2) - (utility:GetTextBounds(name, theme.textsize, theme.font).Y / 2)),
            section.section_frame
        }, {
            Text = name,
            Size = theme.textsize,
        ...
        library.began[#library.began + 1] = function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and window.isVisible and colorpicker_outline.Visible then
                ...
                elseif utility:MouseOverDrawing({
                    section.section_frame.Position.X,
                    section.section_frame.Position.Y + colorpicker.axis,
                    section.section_frame.Position.X + section.section_frame.Size.X - (colorpicker.secondColorpicker and (30 + 4) or 0),
                    section.section_frame.Position.Y + colorpicker.axis + 15
                }) and not window:IsOverContent() then
                    if not colorpicker.open then
                        window:CloseContent()
        ...
        function colorpicker:Colorpicker(info)
            ...
            library.began[#library.began + 1] = function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 and window.isVisible and colorpicker_outline.Visible then
                    ...
                    elseif utility:MouseOverDrawing({
                        section.section_frame.Position.X + (section.section_frame.Size.X - (30 + 4 + 2)),
                        section.section_frame.Position.Y + colorpicker.axis,
                        section.section_frame.Position.X + section.section_frame.Size.X,
                        section.section_frame.Position.Y + colorpicker.axis + 15
                    }) and not window:IsOverContent() then
    ...
    function sections:ConfigBox(info)
        ...
        library.began[#library.began + 1] = function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and configLoader_outline.Visible and window.isVisible and 
               utility:MouseOverDrawing({
                   section.section_frame.Position.X,

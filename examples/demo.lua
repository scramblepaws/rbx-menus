...
local ok, err = xpcall(build, function(e)
	return Library:CaptureError(e, "demo setup")
end)
if not ok then
	Library:Toast("Open the menu — check your clipboard for the full error.", "error", 12)
else
	Library:Toast("Demo loaded. Drag the title bar to move the window.", "success", 4)
end
